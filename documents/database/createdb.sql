...
CREATE TABLE GameContents( -- Game Contents (game description)
    cid INT,
    gid INT,
    pid INT,
    PRIMARY KEY (cid, gid),
    ...
    FOREIGN KEY (gid) REFERENCES Games(gid)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    FOREIGN KEY(pid) REFERENCES GamePhotos(gpid)
        ON DELETE CASCADE
        ON UPDATE CASCADE
);
...
-- Populate Games
INSERT INTO Games(title, did, price) VALUES    
    ('Dota 2', (SELECT bid FROM Business WHERE name='Steam'), 0),
    ('Fire Emblem: Three Houses', (SELECT bid FROM Business WHERE name='Valve Corporation'), 59.99),
    ('Total War: THREE KINGDOMS', (SELECT bid FROM Business WHERE name='Creative Assembly'), 59.99),
    ('Sekiro: Shadows Die Twice', (SELECT bid FROM Business WHERE name='FromSoftware'), 59.99),
    ('Xenogears', (SELECT bid FROM Business WHERE name='Square Enix'), 9.99);
...
-- Populate Stores
INSERT INTO Stores(bid, mid, ops_hours, street, city, state, zip, country) VALUES    
    ((SELECT bid FROM Business WHERE name='GameStop'), 6, 'Business Hours', '1110 S King Rd #30', 'San Jose', 'CA', '95122', 'US'),
    ((SELECT bid FROM Business WHERE name='GameStop'), 7, 'Weekends', '579 Coleman Ave #90', 'San Jose', 'CA', '95110', 'US'),
    ((SELECT bid FROM Business WHERE name='SJSU'), NULL, 'Online Platform', '<Web URL>', NULL, NULL, NULL, NULL);

-- Populate Inventories
INSERT INTO Inventories VALUES	
	(1, 'Fire Emblem: Three Houses', 'B05', 0),
	(1, 'Total War: THREE KINGDOMS', 'B06', 0.3),
	(1, 'Sekiro: Shadows Die Twice', 'B08', 0),
	(1, 'Xenogears', 'B10', 0),
	(2, 'Fire Emblem: Three Houses', 'B05', 0),
	(2, 'Sekiro: Shadows Die Twice', 'B08', 0.5),
	(3, 'Dota 2', 'B04', 0),
	(3, 'Total War: THREE KINGDOMS', 'B06', 0.15),
	(3, 'Sekiro: Shadows Die Twice', 'B08', 0),
	(3, 'Xenogears', 'B10', 0);
